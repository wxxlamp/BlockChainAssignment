...
contract FanEngagementSystemTest is FanEngagementSystem {
    ...
    function testTotolSupplyTokens() public {
        uint256 supplyTokens = super.totalSupply();
        Assert.equal(
            supplyTokens,
            1000000 * 10**18,
            "initial tokens should be 1000000000000000000000000 tokens"
        );
        Assert.equal(
            super.balanceOf(testOwner),
            supplyTokens,
            "test owner has the total supply tokens"
        );
    }
    ...
    function testEarnTokensWithoutOwner() public {
        bool r;
        (r, ) = address(this).call{value: 1 ether}(
    ...
    function testTransferTokens() public {
        ...
        Assert.equal(
            fan1Balance,
            50,
            "Fan 1 should have 50 tokens after transfer"
        );
    }
    ...
    function testRedeemTokens() public payable {
        ...
        Assert.equal(
            balance,
            40,
            "Fan 1 should have 40 tokens after redemption"
        ); // todo
        ...
        Assert.equal(
            history[0],
            "Merchandise",
            "Reward history should contain the redeemed reward"
        );
    }
    ...
    function testMintNFTBadge() public {
        ...
        Assert.equal(
            badgeOwner,
            fan1,
            "Fan 1 should have received an NFT badge"
        );
    }
    ...
    function testSubmitProposal() public {
        ...
        Assert.equal(
            proposalId,
            0,
            "Proposal ID should be 0 for the first proposal"
        );
    }
    ...
    function testSubmitProposalRequiresTokens() public {
        ...
        Assert.ok(
            !r,
            "Should not allow submiting proposal if no tokens are held"
        ); // todo
    }
    ...
    function testVoteRequiresTokens() public {
        bool r;
        (r, ) = address(this).call(
            abi.encodeWithSignature("voteOnProposal(uint256)", 0)
        );
